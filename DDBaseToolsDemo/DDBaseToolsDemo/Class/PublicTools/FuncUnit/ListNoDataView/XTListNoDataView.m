...
@implementation XTListNoDataView

+ (void)showView {
    
    if ([ViewTools sharedInstance].listNoDataView ){
          [[ViewTools sharedInstance].listNoDataView removeFromSuperview];
      }
    
    XTListNoDataView *hh = [[XTListNoDataView alloc] initWithFrame:CGRectZero];
    UIView *currentView =  [ViewTools sharedInstance].currentControllerView;
    [currentView addSubview:hh];
    [ViewTools sharedInstance].listNoDataView = hh;
...
- (void)addChildView {
    ...
    UILabel *tipLabel = [UILabel labelWithText:@"暂无数据点击重新加载" font:17 textColorStr:@"0x888888"];
